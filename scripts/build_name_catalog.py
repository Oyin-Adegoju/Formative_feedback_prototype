"""Eenmalig script: leest de roster-CSV en schrijft people_catalog.json.
...
def _alias_set_voor_naam(
    ...
    # Voornaam + initiaal van kern-achternaam, bv. "blabla d." of "blabla b.".
...
def _normalize_role(rol: str) -> tuple[str | None, bool]:
    """Normaliseert de rol-string.

    Returnt (genormaliseerde_rol, was_herkend):
      - 'lerende' / 'student' → ('Lerende', True)
      - 'docent' / 'leerkracht' → ('Docent', True)
      - leeg/whitespace → (None, False)
      - andere waarde → (originele_waarde, False)  — caller logt warning
    """
    r = rol.strip()
    if not r:
        return None, False
